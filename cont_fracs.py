...
class PiContFrac(basic_algo.PiBasicAlgo):
    def __init__(self, a_coeffs, b_coeffs, diff_mat_gen=None, first_diff_mat=None, with_grad=False, target_val=None, dtype=dec):
    ...
    def _autogen_first_diff_mat_no_grad(self, params, dtype=None):
        ...
        # b_coeffs = params['b_coeffs']
        first_diff_mat = ([], [])
        first_diff_mat[0].append(np.matrix(((dec(a_coeffs[0]),), (dec(1),),), dtype=dtype))
        first_diff_mat[1].append(np.matrix(((dec(1),), (dec(0),)), dtype=dtype))
        return first_diff_mat
